        ...
        public static void RegisterForDebug(this Feedback feedback, IKeyed parent)
        {
            feedback.OutputChange += (sender, args) =>
            {
                if (sender is BoolFeedback)
                    Debug.Console(0, parent, "Received {0} Update : '{1}'", feedback.Key, args.BoolValue);

                if (sender is IntFeedback)
                    Debug.Console(0, parent, "Received {0} Update : '{1}'", feedback.Key, args.IntValue);

                if (sender is StringFeedback)
                    Debug.Console(0, parent, "Received {0} Update : '{1}'", feedback.Key, args.StringValue);
            };
        }
    }
}