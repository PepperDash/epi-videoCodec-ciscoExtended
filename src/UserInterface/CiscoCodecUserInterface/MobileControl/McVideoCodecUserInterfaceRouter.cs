	...
	internal class McVideoCodecUserInterfaceRouter : IKeyed
	{
		...
		)
		{
			try
			{
				...
				if (uimap == null) { Debug.LogMessage(LogEventLevel.Debug, "uimap is null", null, null); return; }
				var thisUisDefaultRoomKey = _mcTpController?.DefaultRoomKey;
				Debug.LogMessage(LogEventLevel.Debug, $"thisUisDefaultRoomKey: {thisUisDefaultRoomKey}", null, null);
				var thisUisUiMapRoomKeyValue = (
					uimap?.FirstOrDefault((kv) => kv.Key == thisUisDefaultRoomKey)
				)?.Value;
				Debug.LogMessage(LogEventLevel.Debug, $"thisUisUiMapRoomKeyValue: {thisUisUiMapRoomKeyValue}", null, null);
				var primaryRoomKey = "";
				var getPrimaryKeySuccess = uimap?.TryGetValue("primary", out primaryRoomKey);
				Debug.LogMessage(LogEventLevel.Debug, $"primaryRoomKey done: {primaryRoomKey}", null, null);
